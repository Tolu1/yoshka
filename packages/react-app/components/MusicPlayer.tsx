...
function MusicPlayer() {
  ...
  const songSavedRef = useRef<HTMLDivElement | null>(null);
  const songPercentagePlayedRef = useRef<HTMLInputElement | null>(null);

      // useEffect for theme handling
  useEffect(() => {
    console.log(theme)
    if (theme === 'dark') {
  ...
  // useEffect for space key event listener
  useEffect(() => {
    ...
    Amplitude.init({
      ...
      callbacks: {
        timeupdate: function () {
          ...
          if (songPercentagePlayedRef.current) {
            songPercentagePlayedRef.current.style.backgroundSize = `${percentage}% 100%`;
          }
        },
      },
      songs: [
        {
          ...
          "artist": "markotopa",
          "album": "Single",
          "url": "/assets/songs/no-copyright/a-call-to-the-soul.mp3",
          "cover_art_url": "/assets/songs/no-copyright/a-call-to-the-soul.jpg"
        },
        {
          "name": "First Snow",
          "artist": "Emancipator",
  ...
  const toggleSongSaved = () => {
    if (songSavedRef.current) {
      songSavedRef.current.classList.toggle('saved');
    }
  };
  
  return (
    // <div classNameName="px-7 py-4">
    //   <div classNameName="flex text-lg sm:text-xl md:text-2xl font-bold text-gray-300">
    //       <span classNameName='py-2'>Music player Section...</span>
    //   </div>
    // </div>
    <main id='musicPlayer'>
    <section className="">
		<div className="bg-[#0A1122]/50 w-full h-screen flex flex-col items-center justify-center">
      {/* <div className="flex text-lg sm:text-xl md:text-2xl font-bold text-gray-300">
          <span className='py-4'>Skip to your song</span>
      </div> */}
			<div className="relative w-player flex flex-col rounded-xl shadow-player-dark bg-player-dark-background border border-player-dark-border backdrop-blur-xl">
				<div className="px-10 pt-10 pb-4 flex items-center z-50">
					<img data-amplitude-song-info="cover_art_url" className="w-24 h-24 rounded-md mr-6 border border-cover-dark-border"/>

					<div className="flex flex-col">
						<span data-amplitude-song-info="name" className="font-sans text-lg font-medium leading-7 text-white"></span>
						<span data-amplitude-song-info="artist" className="font-sans text-base font-medium leading-6 text-gray-400"></span>
						<span data-amplitude-song-info="album" className="font-sans text-base font-medium leading-6 text-gray-400"></span>
					</div>
				</div>
				<div className="w-full flex flex-col px-10 pb-6 z-50">
					<input ref={songPercentagePlayedRef} type="range" id="song-percentage-played" className="amplitude-song-slider mb-3" step=".1"/>
				...
				<div className="h-control-panel px-10 rounded-b-xl border-t flex items-center justify-between z-50 bg-control-panel-dark-background border-gray-900">
					<div onClick={toggleSongSaved} ref={songSavedRef} className="cursor-pointer" id="song-saved">
					...
					<div className="cursor-pointer amplitude-play-pause w-24 h-24 rounded-full border shadow-xl flex items-center justify-center bg-play-pause-dark-background border-play-pause-dark-border">
						<svg id="play-icon" className="ml-[10px]" width="31" height="37" viewBox="0 0 31 37" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path fill-rule="evenodd" clip-rule="evenodd" d="M29.6901 16.6608L4.00209 0.747111C2.12875 -0.476923 0.599998 0.421814 0.599998 2.75545V33.643C0.599998 35.9728 2.12747 36.8805 4.00209 35.6514L29.6901 19.7402C29.6901 19.7402 30.6043 19.0973 30.6043 18.2012C30.6043 17.3024 29.6901 16.6608 29.6901 16.6608Z" className="fill-slate-400"/>
						</svg>	
													
						<svg id="pause-icon" width="24" height="36" viewBox="0 0 24 36" fill="none" xmlns="http://www.w3.org/2000/svg">
							<rect width="6" height="36" rx="3" className="fill-slate-400"/>
							<rect x="18" width="6" height="36" rx="3" className="fill-slate-400" />
						</svg>	
					</div>
				...
				<div className="block top-14 w-full absolute ml-auto mr-auto left-0 right-0 text-center max-w-lg h-72 rounded-full bg-highlight blur-2xl">
